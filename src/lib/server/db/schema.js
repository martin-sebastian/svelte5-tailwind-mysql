...
export const accpack = mysqlTable(
	'accpack',
	{
		...
		maped: boolean('maped').notNull().default(false),
		autorv: boolean('autorv').notNull().default(false),
...
export const accpackmu = mysqlTable(
	...
	(table) => {
		return {
			accpackmu_accpack: index('accpackmu_accpack').on(table.accpackid),
			accpackmu_mu: index('accpackmu_mu').on(table.muid)
		};
	}
);
...
export const mu = mysqlTable(
	'mu',
	{
		...
		maped: boolean('maped').notNull().default(false),
		yellowtag: boolean('yellowtag').notNull().default(false),
		recnew: boolean('recnew').notNull().default(false),
		printreq: boolean('printreq').notNull().default(false),
		hotdeal: boolean('hotdeal').notNull().default(false)
	},
	(table) => {
		return {
...
export const mysalecommitem = mysqlTable(
	'mysalecommitem',
	{
		...
		listnumber: int('listnumber').notNull(),
		itemobtained: boolean('itemobtained').notNull().default(false)
	},
	(table) => {
		return {
